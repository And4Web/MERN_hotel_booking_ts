...
test("Should show hotel Details", async ({page})=>{
  ...
  await page.getByText("premium hotel").click();
...
test("should book hotel", async({page})=>{
  ...
  await expect(page.getByText("Total Cost: ₹4797.00")).toBeVisible();

  const stripeFrame = page.frameLocator("iframe").first();

  await stripeFrame.locator('[placeholder="Card number"]').fill("4242424242424242");
  await stripeFrame.locator('[placeholder="MM / YY"]').fill("10/50");
  await stripeFrame.locator('[placeholder="CVC"]').fill("255");
  await stripeFrame.locator('[placeholder="ZIP"]').fill("25542");
